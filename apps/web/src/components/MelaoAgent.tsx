...
import { X, Send, Mic, Music2, Sparkles } from 'lucide-react';
  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed bottom-24 right-5 z-50 w-14 h-14 rounded-full shadow-2xl flex items-center justify-center transition-transform hover:scale-110 active:scale-95"
      ...
      {/* Chat panel */}
      {open && (
        <div className="fixed bottom-24 right-5 z-50 w-80 rounded-2xl shadow-2xl flex flex-col overflow-hidden border border-[#333]"
          ...
          <div className="flex items-center gap-3 px-4 py-3 border-b border-[#222]"
            ...
            <div className="flex items-center gap-1">
              ...
              <button onClick={() => setOpen(false)} className="text-gray-500 hover:text-white transition-colors ml-1" aria-label="Close">
                <X className="w-4 h-4" />
              </button>
            </div>
          </div>
          ...
          {/* Input */}
          <div className="px-3 py-2 border-t border-[#222] flex items-center gap-2">
            <input
              value={input}
            ...
            <button onClick={send} disabled={!input.trim()}
              className="w-8 h-8 rounded-full flex items-center justify-center transition-all disabled:opacity-30 hover:scale-110 active:scale-95 flex-shrink-0"
              style={{ background: 'linear-gradient(135deg, #F28C28, #E91E8C)' }}
